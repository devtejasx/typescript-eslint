...
### Customizing a shared config

Our shared configs are a starting point, not an all-or-nothing choice.
To change a rule they enable, or to enable one they do not, add a config object after the `extends`.
Later config objects win, so its `rules` are applied on top of everything the shared configs set.
...
There is no need to repeat `plugins` or `languageOptions.parser`: the shared configs already declare both, and the rules you add are looked up in the plugin they registered.

Enabling a [type-aware rule](../getting-started/Typed_Linting.mdx) that a non-type-aware shared config does not turn on takes one more step, because those rules need type information:
...
:::tip
Scope the config object with `files` when you enable type-aware rules this way.
Without it the parser options apply to every file ESLint looks at, including ones your TSConfig does not include, and those files then fail to parse.
See [Typed Linting](../getting-started/Typed_Linting.mdx) for the full setup and for the shared configs that enable type-aware rules for you.
:::
